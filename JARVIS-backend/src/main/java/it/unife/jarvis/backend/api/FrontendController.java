...
public class FrontendController {
    ...
    public ResponseEntity<?> getEventInfo () {
        try {
            EventInfo output = consumer.consumeRESTParse("http://mock:3000/api/mock_supplier", EventInfo.class);
    ...
    public ResponseEntity<?> getEventsInfo () {
        try {
            ...
            for (int i = 0; i < 5; i++) {
                EventInfo e = consumer.consumeRESTParse("http://mock:3000/api/mock_supplier", EventInfo.class);
                events.add(e);
            }
    ...
    public ResponseEntity<?> getUser () {
        try {
            String output = consumer.consumeRESTParse("http://mock:3000/api/fetchUser", String.class);
            return ResponseEntity.ok(output);
        }
        catch(Exception exc) {
            exc.printStackTrace();
